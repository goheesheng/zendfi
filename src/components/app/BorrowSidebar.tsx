...
export function BorrowSidebar() {
  ...
  return (
    <div className="space-y-3 mt-6">
      {/* How It Works */}
      <div className="bg-white border border-zend-accent-soft rounded-xl p-4">
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-4">
        ...
        {activeLoans.length === 0 ? (
          <div className="text-center py-4">
            ...
            <p className="text-[11px] text-gray-300 mt-1">Your loans will appear here after borrowing</p>
          </div>
        ) : (
          <div className="space-y-2">
            {activeLoans.slice(0, 3).map((loan) => (
              <div key={loan.quotationId.toString()} className="flex items-center justify-between py-1.5 border-b border-gray-50 last:border-0">
                <div className="flex items-center gap-2">
                  <TokenIcon symbol="ETH" size={16} />
                  <span className="text-xs font-medium text-gray-700">Loan #{loan.quotationId.toString()}</span>
                </div>
                <span className="text-[11px] text-gray-400 capitalize">{loan.status}</span>
              </div>
            ))}
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-4">
        <h3 className="text-sm font-semibold text-gray-900 mb-3">Supported Collateral</h3>
        <div className="flex gap-3">
          <div className="flex items-center gap-2 bg-gray-50 rounded-lg px-3 py-2 flex-1">
            <TokenIcon symbol="ETH" size={20} />
            <div>
              <div className="text-xs font-semibold text-gray-900">ETH</div>
              <div className="text-[10px] text-gray-400">Wrapped Ether</div>
            </div>
          </div>
          <div className="flex items-center gap-2 bg-gray-50 rounded-lg px-3 py-2 flex-1">
            <TokenIcon symbol="BTC" size={20} />
            <div>
              <div className="text-xs font-semibold text-gray-900">cbBTC</div>
              <div className="text-[10px] text-gray-400">Coinbase BTC</div>
            </div>
          </div>
        </div>
      </div>
      ...
      {/* Key Facts */}
      <div className="bg-white border border-gray-200 rounded-xl p-4">
        <h3 className="text-sm font-semibold text-gray-900 mb-3">Why ZendFi?</h3>
        <div className="space-y-2">
          {[
            ...
            <div key={item.label} className="flex items-start gap-2">
              ...
              <div>
                ...
                <span className="text-[11px] text-gray-400 ml-1">{item.desc}</span>
              </div>
            </div>
          ))}
        </div>
