  ...
  return (
    <Modal open={open} onClose={onClose} title="Settings">
      <div className="space-y-6">
        <div>
          <label className="text-sm font-medium text-gray-700 block mb-2">Show options expiring in (days)</label>
          <input type="number" value={minDays} onChange={(e) => setMinDays(Number(e.target.value))} className="w-full px-3 py-2 rounded-lg bg-gray-50 border border-gray-200 text-gray-900 text-sm" />
        </div>
        <div>
          <label className="text-sm font-medium text-gray-700 block mb-2">Max strikes per expiry</label>
          <input type="number" value={maxStrikes} onChange={(e) => setMaxStrikes(Number(e.target.value))} className="w-full px-3 py-2 rounded-lg bg-gray-50 border border-gray-200 text-gray-900 text-sm" />
        </div>
        <div>
        ...
        <div>
          <label className="text-sm font-medium text-gray-700 block mb-2">Maximum APR: {maxApr}%</label>
        ...
        <div className="flex items-center justify-between">
          <label className="text-sm font-medium text-gray-700">Keep order open</label>
          <button onClick={() => setKeepOpen(!keepOpen)} className={`w-11 h-6 rounded-full transition-colors ${keepOpen ? 'bg-zend-blue' : 'bg-gray-300'}`}>
