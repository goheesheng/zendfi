    ...
    <div className="space-y-6">
      <div className="flex justify-center py-8">
        {isCompleted ? (
          ...
          <div className="w-24 h-24 rounded-full bg-zend-blue/10 flex items-center justify-center animate-pulse">
            <svg className="w-12 h-12 text-zend-blue" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
          </div>
        )}
      </div>
            ...
            <div key={i} className="flex items-center justify-between p-3 bg-gray-50 dark:bg-zend-bg rounded-xl border border-gray-200 dark:border-zend-border">
              ...
              <button
                onClick={async () => {
                  ...
                  try {
                    await service.acceptOffer(BigInt(state.activeLoanRequestId!), offer.decryptedAmount, BigInt(offer.nonce), offer.offeror);
                ...
                className="px-4 py-1.5 rounded-lg bg-zend-blue text-white text-xs font-semibold hover:bg-zend-blue-dark disabled:opacity-40 transition-colors"
              >
                Accept
              </button>
            </div>
      ...
      <div className="pt-4">
        {isCompleted ? (
          <button onClick={onDismiss} className="w-full py-3 rounded-xl font-semibold text-white bg-zend-blue hover:bg-zend-blue-dark transition-all shadow-[0_4px_16px_rgba(93,116,255,0.3)]">
            View Loan Details
          </button>
        ) : (
